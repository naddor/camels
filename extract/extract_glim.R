...
load(file='/home/naddor/data/shp_671_catch_wgs84.Rdata')
...
# load geol data in R format
load('/d7/naddor/data/geol/limw_wgs84_us.Rdata')
glim_classes=data.frame(short_name=limw_wgs84_us@data$xx,order=1:length(limw_wgs84_us@data$xx))
...
#pdf(paste0(dir_plots,'camels/geol/glim_us.pdf',sep=''),width=20,height=16)

#plot(limw_wgs84_us,col=as.character(glim_classes_col$R_col),border=NA)
...
pdf(paste0(dir_plots,'camels/geol/glim_us_catch_test.pdf',sep=''),width=20,height=11)

plot(limw_wgs84_us,col=as.character(glim_classes_col$R_col),border=NA)
map('state',col='black',lwd=1,add=TRUE)
#map('worldHires','USA',add=TRUE)

legend('bottomright',col=as.character(table_glim_classes$R_col),legend=table_glim_classes$long_name,ncol=2,pch=15,bty='n',cex=0.7)

for(e in 1:nrow(catch_topo)){

  #589 12041200     17         HOH RIVER AT US HIGHWAY 101 NEAR FORKS, WA
  catch_id=catch_topo$gage_id[e]
  print(paste(e,catch_name$gage_name[e]))

  # determime area of polygons overlapping with each catchment
  inter<-intersect(limw_wgs84_us,shp_671_catch[shp_671_catch@data$gauge_id==catch_id,])
  ...
  plot(shp_671_catch[shp_671_catch@data$gauge_id==catch_id,],add=TRUE,
         col=NA,border='black',lty=1,lwd=0.4)

  plot(shp_671_catch[shp_671_catch@data$gauge_id==catch_id,],add=TRUE,
       col=NA,border=as.character(table_glim_classes$R_color[table_glim_classes$long_name==names(dom_geol[1])]),lty=1,lwd=0.2)
...
glim_1st_frac<-round(glim_1nd_frac,3)
...
save(catch_geol_glim,file=paste(dir_catch_attr_temp,'catch_geol_glim.Rdata',sep=''))

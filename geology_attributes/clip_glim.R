...
### LiMW
fgdb <- file.path(dir_data, 'LiMW_GIS_2015.gdb')
fc <- readOGR(dsn = fgdb, layer = "GLiM_export")

# Change projection system
fc_wgs84 <- spTransform(fc, CRS("+proj=longlat +datum=WGS84"))
save.image(file.path(dir_data, 'LiMW.Rdata'))

# Crop to regional domains
limw_wgs84_us <- crop(fc_wgs84, extent(-125, -66, 23, 50))
save(limw_wgs84_us, file = file.path(dir_data, 'limw_wgs84_us.Rdata'))

limw_wgs84_uk <- crop(fc_wgs84, extent(-11, 2.5, 49, 59))
save(limw_wgs84_uk, file = file.path(dir_data, 'limw_wgs84_uk.Rdata'))

limw_wgs84_es <- crop(fc_wgs84, extent(-10, 3.5, 35.5, 44))
save(limw_wgs84_es, file = file.path(dir_data, 'limw_wgs84_es.Rdata'))

limw_wgs84_cl <- crop(fc_wgs84, extent(-76, -64, -56, -16.5))
save(limw_wgs84_cl, file = file.path(dir_data, 'limw_wgs84_cl.Rdata'))

limw_wgs84_us_pnw <- crop(fc_wgs84, extent(-125, -120, 45, 50))
save(limw_wgs84_us_pnw, file = file.path(dir_data, 'limw_wgs84_us_pnw.Rdata'))
